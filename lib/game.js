...
export class CinchGame {
  ...
  constructor () {
    ...
    this.trickPlays = []
    this.lastTrickPlays = [] // Store last completed trick for display after hand ends
  ...
  startNewHand () {
    ...
    this.trickPlays = []
    this.lastTrickPlays = [] // Clear last trick from previous hand
    this.bids = 0
    this.playersDiscarded = 0
    this.cinchBidder = null
    this.cinchOverridePhase = false
  ...
  dealNewCards () {
    for (const player of this.players) {
      ...
      // Sort the player's hand after dealing new cards
      player.hand.sort()
    }
  }
  ...
  /**
   * Gets all players on the opposing team who haven't bid yet and are eligible to override.
   * Used for cinch override logic.
   * Only the dealer can override a cinch bid.
   * @returns {Player[]} Array of eligible opposing players (only dealer if on opposing team)
   */
  getEligibleOpposingPlayers () {
    const cinchBidderTeam = this.cinchBidder.team

    // Check if dealer is valid
    ...
    // Only the dealer can override, and only if they're on the opposing team and haven't bid
    if (dealer.team !== cinchBidderTeam && !this.playersBid.has(dealer.seat)) {
      return [dealer]
    }
  ...
  playCard (player, cardIndex) {
    ...
    if (this.trickPlays.length === 4) {
      ...
      // Save the last trick before clearing for display after hand ends
   ...
   */
  isGameComplete () {
    // Game is complete when at least one team has reached 21 AND there's no tie
    // If both teams are at 21 or above, game continues until one team has more points
    if (this.scores.team1 >= 21 || this.scores.team2 >= 21) {
      // If scores are tied, game continues
      if (this.scores.team1 === this.scores.team2) {
        return false
      }
      // If not tied and at least one team is at 21+, game is complete
      return true
    }
    return false
  }
   ...
   */
  getWinningTeam () {
    // Only return a winner if the game is actually complete
    if (!this.isGameComplete()) {
      return null
    }

    // At this point we know at least one team is >= 21 and they're not tied
    if (this.scores.team1 > this.scores.team2) {
      return 1
    } else {
      return 2
